...
# Endpoint para servir imágenes desde el directorio static/qr_code
@main.route('/api/static/qr_codes/<path:filename>')
def serve_qr_code(filename):
    # Usar os.path.join para construir la ruta completa de manera segura
    qr_file_path = os.path.join(os.getcwd(), 'static', 'qr_codes')

    # Imprimir la ruta para depuración
    print("Ruta completa QR Code:", qr_file_path)

    # Usar send_from_directory para servir el archivo desde el directorio 'qr_codes'
    return send_from_directory(qr_file_path, filename)

# Endpoint para servir imágenes desde el directorio static/creatures
@main.route('/api/static/creatures/<path:filename>')
def serve_creature_image(filename):
    
    # Construir la ruta completa al directorio "creatures"
    creatures_file_path = os.path.join(os.getcwd(), 'static', 'creatures')
   
    # Imprimir la ruta para depuración
    print("Ruta completa Image Creature:", creatures_file_path)


    # Verificar si el archivo existe
    full_file_path = os.path.join(creatures_file_path, filename)
...
def generate_creature():
    try:
        ...
        try:
            ...
            return jsonify({"error": "An error occurred while generating the image of creature."}), 500
...
def buy_creature():    
    try:
        ...
        return jsonify({
            'message': 'Creature and Story save in Database',
